...
class AxaioMadeToPrint_WflSetObjectProperties extends WflSetObjectProperties_EnterpriseConnector
{
	...
	final public function runBefore( WflSetObjectPropertiesRequest &$req )
	{
        ...
        if (LogHandler::debugMode()) {
            LogHandler::Log( 'AxaioMadeToPrint', 'DEBUG', '$req: ' . print_r($req, true) );
        }
	...
	final public function runAfter( WflSetObjectPropertiesRequest $req, WflSetObjectPropertiesResponse &$resp )
	{
        ...
        if (LogHandler::debugMode()) {
            LogHandler::Log( 'AxaioMadeToPrint', 'DEBUG', '$req: ' . print_r($req, true) );
            LogHandler::Log( 'AxaioMadeToPrint', 'DEBUG', '$resp: ' . print_r($resp, true) );
        }
        require_once dirname(__FILE__) . '/AxaioMadeToPrintDispatcher.class.php';
...
#        LogHandler::Log( 'AxaioMadeToPrint', 'ERROR', $resp->MetaData->BasicMetaData->Type);
        
        if( defined( "AXAIO_MTP_TRIGGER_ON_CHANGE_ONLY") && AXAIO_MTP_TRIGGER_ON_CHANGE_ONLY == true) {
            if( $this->prevStatusId && (substr($resp->MetaData->BasicMetaData->Type, 0, 6) == 'Layout') ) { //set and other than 0 and is layout
                $currentStatusId = (isset( $req->MetaData->WorkflowMetaData->State->Id))
                                         ? $req->MetaData->WorkflowMetaData->State->Id
                                         : 0;
                ...
                if($currentStatusId != $this->prevStatusId) {
                    ...
                    LogHandler::Log('AxaioMadeToPrint', 'DEBUG', 'Skipping layout '.$req->ID.' because the status was not changed' );
                }
            }
        } else {
            if((BizSession::getShortUserName() == AXAIO_MTP_USER) && ($req->MetaData->WorkflowMetaData->State->Id == $resp->MetaData->WorkflowMetaData->State->Id) ){
                // do not print
            }else{
                AxaioMadeToPrintDispatcher::doPrint( $resp->MetaData->BasicMetaData->ID, $ticket );
            }
