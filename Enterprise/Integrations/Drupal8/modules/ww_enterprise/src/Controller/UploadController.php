 ...
 *
 * See ww_enterprise.routing.yml, the route defined there leads to this
  ...
  public function upload() {
    ...
    try {
      ...
      $fields = \Drupal::entityManager()
        ->getFieldDefinitions('node', $originalType);
        ...
        if ($field instanceof \Drupal\field\Entity\FieldConfig) {
          ...
          if ($field->getName() == $field_machine_name) {
            ...
            if ($fieldType != 'text_long' && $fieldType != 'text_with_summary') {
              ...
              $scheme = $field->getFieldStorageDefinition()
      ...
      if (!is_dir($path)) {
        \Drupal::service('file_system')->mkdir($path, NULL, TRUE);
      }
      ...
      \Drupal::logger('ww_enterprise')
        ->error('Uploading a file failed: ' . $e->getMessage());
