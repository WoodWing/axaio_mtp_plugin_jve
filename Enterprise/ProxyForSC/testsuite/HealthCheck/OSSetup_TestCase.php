 ...
 *
 * It checks if the operating system is configured well and permissions and settings are ok.
...
class WW_TestSuite_HealthCheck_OSSetup_TestCase extends TestCase
{
	...
	*/
	private function getTempFileName()
	{
		...
		if( !$this->tmpFile ) {
			...
			$this->setResult( 'ERROR',  'The proxy could not generate temporary file at "'.$this->tmpDir.'" folder. ' , $help );	
			return false;
		}
	...
	*/
	private function writeToTemporaryFile()
	{
		...
		if ($result === false) {
			$help = 'Please check if www/inet user has write access to the system temp folder.';
			$this->setResult( 'ERROR',  "The proxy could not write to temporary file $this->tmpFile. " , $help );		
		}
	...
	*/
	private function readTemporaryFile()
	{
		...
		if (!$result) {
			...
			$this->setResult( 'ERROR',  "The proxy could not read the temporary file $this->tmpFile. " , $help );		
		}
	...
	*/
	private function writeToTemporaryFileExternal()
	{
		...
		if (filesize( $this->tmpFile ) == 0 ) {
			$this->setResult( 'ERROR',  "The proxy could not write to temporary file $this->tmpFile by external command shell_exec($command)." , $help );	
			return false;
		}	
	...
	*/
	private function deleteTempFileExternal()
	{
		...
		if (file_exists( $this->tmpFile )) {
			$this->setResult( 'ERROR',  "The proxy could not delete the temporary file {$this->tmpFile} by external command shell_exec($command)." , $help );	
			return false;
		}	
	...
	*/
	private function deleteTempFile()
	{
		...
		if (!$result) {
			$this->setResult( 'ERROR',  "The proxy could not delete temporary file {$this->tmpFile}." , $help );		
		}
	}
}
