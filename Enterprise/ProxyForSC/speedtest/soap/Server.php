...
class WW_DIME_Record
{
	...
	public function read ($handle)
	{
		...
		if ($data === FALSE || strlen( $data ) < self::HEADER_LEN) {
			throw new Exception( 'Could not read or invalid DIME record' );
		}
		...
		if ($padOptsIdTypeLen > 0) {
			...
			if ($data === FALSE) {
				throw new Exception( 'Could not read options from DIME record' );
			}
			$p = 0;
...
class WW_SOAP_Server
{
	...
	static public function getWsdlCacheFolder()
	{
		...
		if( $cacheFolder ) {
			...
			$version = str_replace( array( ' ', '.' ), '_', PRODUCT_VERSION );
			$subFolder = '/ww_ent_speedtest_'.$version;
	...
	public function handle ()
	{
		...
		if (strpos( $response, '<faultcode>' ) === FALSE && ! WW_SOAP_Service::usedReturnResponse()) {
			throw new Exception( 'Service class ' . $this->className . ' has not used returnResponse function' );
		}
	...
	public function setClass ($className)
	{
		...
		if (! is_subclass_of( $className, 'WW_SOAP_Service' )) {
			throw new Exception( 'Error setting classname ' . $className );
		}
	...
	private function getSoapAction ($soapRequest)
	{
		...
		if (sizeof( $soapActs ) <= 2) {
			throw new Exception( 'The SOAP action was not found in envelope. Request = ' . $soapRequest );
		}
