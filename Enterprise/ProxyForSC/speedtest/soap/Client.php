...
require_once BASEDIR . '/speedtest/soap/Server.php';
...
// 		}
	...
	private function openSocketConnection( $location, &$urlParts )
	{
		...
		if( $urlParts['scheme'] == 'https' ) {
			// BZ#17043 Support for SSL, we only need to add ssl:// in front of host ;)
			$host = 'ssl://' . $host;
		}
		...
		if( !$fh ) { // failed to open
			throw new Exception( "$errstr ($errno)", "Failed to connect to $location." );
		}
		return $fh;
	}
	...
	function __readResponse(&$fh)
	{
        ...
        if( $resType == 4 || $resType == 5 ) { // is http error?
			...
			throw new Exception( $header . ' Fatal communication error with SOAP server: '.$httpErrMSg.' ('.$code.')' );
		}
	...
	private function replaceHref ($soapXML)
	{
		...
		foreach ($nodes as $node) {
			...
			if (! is_null( $record )) {
				...
				throw new Exception( 'DIME record ' . $id . ' not found' );
			}
		}
