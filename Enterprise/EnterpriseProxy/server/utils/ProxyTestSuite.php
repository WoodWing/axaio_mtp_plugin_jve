...
class WW_Utils_ProxyTestSuite
{
	...
	public function dirPathExists( $filePath )
	{
		$realPath = null;
		if( OS == 'WIN' ) {
			// BZ#30405 - PHP realpath failed on UNC path, it returning the UNC path in uppercase, which failed in comparison
			// Use is_dir to check UNC path, and use realpath to test local path
			if( substr( $filePath, 0, 2 ) == '//' ) { // Network UNC path
	 ...
	 * Does logon the TESTSUITE user at Entprise Server (by using LogOn web service).
	 *
	 * @since 9.0.0
	 * @param TestCase $testCase The test module calling this function.
