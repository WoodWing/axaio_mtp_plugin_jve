...
//    To automatically clean up the directory, see 'AUTOCLEAN_WEBEDITDIR_DAYS'.
//
...
//  Automatically deletes Enterprise Service Logs entries in the smart_log table when they become older than a specified
//  number of days.

// AUTOCLEAN_SERVICELOGS_DAYS
//    When LOGLEVEL is set to any value other than 0, service logging is enabled.
//    This leads to continuous data growth in the smart_log table.
//    To periodically clean this table, AUTOCLEAN_SERVICELOGS_DAYS can be set to any value other than 0.
//    The default value is 30 which means that all records in the smart_log table that are older than 30 days will be
//    deleted.
//    Setting it to 0 will disable the cleanup of the table.
if( !defined('AUTOCLEAN_SERVICELOGS_DAYS') ) {
	define( 'AUTOCLEAN_SERVICELOGS_DAYS', 30 );
}

// -------------------------------------------------------------------------------------------------
//  Enterprise WEBEDIT directory Auto Cleanup
// -------------------------------------------------------------------------------------------------

//  Automatically deletes files (and subdirectories) older than the specified number of days.

// AUTOCLEAN_WEBEDITDIR_DAYS
//    Files in the WEBEDIT directory are not always cleaned up. This can be because InDesign Server crashes or because
//    the article workspace used by Content Station is not properly closed. The directory can also contain log files
//    generated by InDesign Server; these files also take up space.
//    To periodically clean this directory, AUTOCLEAN_WEBEDITDIR_DAYS can be set to any value other than 0.
//    The default value is 7 which means that all files in the directory that are older than 7 days will be
//    deleted.
//    Setting it to 0 will disable the cleanup of the directory.
if( !defined('AUTOCLEAN_WEBEDITDIR_DAYS') ) {
	define( 'AUTOCLEAN_WEBEDITDIR_DAYS', 7 );
}
