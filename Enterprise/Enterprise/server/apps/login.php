...
if( !$loginsucceed ) {
	...
	if( $userLimitAdmin || $licenseAdmin ) {
		...
		if( empty( $sessionId ) ) {
			// If the session id is empty, the session_start() will fail. This is changed in PHP 7.1.
			// In that version the session id is generated on the fly.
			// More info: http://git.php.net/?p=php-src.git;a=commitdiff;h=8c37a086c78a66517967fcb809fb53297becfe42
