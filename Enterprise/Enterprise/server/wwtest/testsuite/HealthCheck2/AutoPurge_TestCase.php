...
class WW_TestSuite_HealthCheck2_AutoPurge_TestCase extends TestCase
{
	...
	/**
	 * Run our test. Called by the core (TestSuiteFactory class).
	 */
	final public function runTest()
	{
		...
		if( !is_null( $response ) ) {
			...
			foreach ( $autoPurgePubs as $pubId => $afterDayForPurging ) {
				if( $afterDayForPurging > 0 ){ // only check if it is configured to 1 day or more (0 means disabled for this particular brand)
					// retrieve deletedObject(s) that stay in trash can longer than the days specified
					...
					require_once BASEDIR . '/server/bizclasses/BizQuery.class.php';
					$obj = BizQuery::queryObjects2( $request, $this->user );
					if( count( $obj->Rows) > 0 ){
	 ...
	 * @return WflLogOnResponse on succes. NULL on error
	 */
	 ...
	 * the deleted objects.
	 *
	 * @return bool True when all the settings are fine, False otherwise.
	 */
