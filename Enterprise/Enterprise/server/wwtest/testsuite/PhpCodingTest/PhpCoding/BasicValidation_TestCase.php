 ...
 *
 * Tests if the include paths of all PHP modules can be resolved that belong to Enterprise Server.
 * The few paths that are built dynamically (based on variables) are skipped.
 * 3rd party includes are skipped too since they are concidered to be fine.
...
class WW_TestSuite_PhpCodingTest_PhpCoding_BasicValidation_TestCase extends TestCase implements FolderIterInterface
{
	...
	public function getDisplayName() { return 'Basic PHP validation'; }
	public function getTestGoals()   { return 'Avoid on-site problems due to commonly made mistakes at PHP coding.'; }
	public function getTestMethods() { return 'Parses PHP sources using the internal tokenizer. Checks for use of deprecated PHP functions, case sensitive include paths for Linux compatibility, etc.'; }
	...
	{
		$level = $level; // keep analyzer happy
	...
	/**
	 * The atucal PHP source file validation. See module header for functionality.
			   			...
			   			if( !empty($incPath) ) { // empty happens for variable path names, which we don't check
							...
							if( !is_readable( $incRealPath ) ) { // try relative to base path of current source file
								...
								if( is_readable( $incRealPath ) ) {
									...
									if( strpos( $pathPart, 'config/config.php' ) === false && // we only allow relative config.php includes
											$pathInfo['basename'] != 'config.php' ) { // we also allow all relative includes made from the config.php file
										$reliesOnSourceBase = true;
									}
									// NOTE: Relative includes must go all the way up to the BASEDIR first, then down again. 
									//       No matter if included from the config subfolder! This is a house rule.
								}
							}
							...
							if( is_readable( $incRealPath ) ) {
								do {
									...
									// This works for PHP 5.3+ since realpath() does no longer resolve upper/lower case filenames
								...
								do {
									...
									if( strpos( $filePath, '/config/plugins/' ) === false ||
										strpos( $incPath, '../../../config/config.php' ) === false ) { // suppress custom plugins doing the logical link trick
										$this->setResult( 'ERROR', $this->getFileInfoStr($filePath).
	...
	/**
	 * Try to detect strange ionCube + Zend Optimizer crash
	 * See BZ#17063
	 *
