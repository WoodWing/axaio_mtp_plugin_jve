...
define ('LOADERS_PACKAGE_PREFIX',''); // WoodWing: all loaders are in one package and its subfolders have NO prefix.
...
define ('REQUIRED_LOADER_VERSION', '5.0.14'); // WoodWing: stick to only but this version
...
function loader_install_instructions($server_type,$loader_dir = '')
{
    ...
    echo '<li>Copy the <code>'.$basename.DIRECTORY_SEPARATOR.get_loader_name().'</code> file from the extracted archive '.
         'to the <code>'.$loader_dir.'</code> folder in your Application Server.</li>';
...
function server_restart_instructions()
{
    ...
    echo "<li>After the restart, <a href=\"$base&amp;page=loader_check\" onclick=\"showOverlay();\">refresh the page</a>.</li>";
    // <<<

    if ($sysinfo['SS'] == 'Apache' && !is_ms_windows()) {
    	// >>> WoodWing: removed broken link
        ...
        echo '<p>Note: If the Loader installation fails, check the Apache error log file for errors.</p>';
        // <<<
    }
}
...
{
// >>> WoodWing: We only support local installations.
//     $php_ini_name = ini_file_name();
//     $default = get_default_address();
// 
//     echo '<p>';
...
function loader_compatibility_test($loader_location)
{
    ...
    if (preg_match("/php version:\s*(.)\.(.)\.(..?)(-ts)?/i",$loader_strs,$version_matches)) {
        if ($version_matches[1] != $phpv['major'] || $version_matches[2]  != $phpv['minor']) {
            ...
            $errors[ERROR_LOADER_PHP_MISMATCH] = "The installed loader is for PHP $loader_php but your Web Server is running PHP $server_php.";
        }
        if (is_bool($sysinfo['THREAD_SAFE']) &&  $sysinfo['THREAD_SAFE'] && !is_ms_windows() && !(isset($version_matches[4]) && $version_matches[4] == '-ts')) {
            $errors[ERROR_LOADER_NONTS_PHP_TS] = "Your Web Server is running a thread-safe version of PHP but the loader is not a thread-safe version.";
        } elseif (isset($version_matches[4]) && $version_matches[4] == '-ts' && !(is_bool($sysinfo['THREAD_SAFE']) &&  $sysinfo['THREAD_SAFE'])) {
            $errors[ERROR_LOADER_TS_PHP_NONTS] = "Your Web Server is running a non-thread-safe version of PHP but the loader is a thread-safe version.";
        }
    } elseif (preg_match("/ioncube_loader_.\.._(.)\.(.)\.(..?)(_nonts)?(_amd64)?\.dll/i",$loader_strs,$version_matches)) {
        if (!is_ms_windows()) {
            $errors[ERROR_LOADER_WIN_SERVER_NONWIN] = "You have a Windows loader but your Web Server does not appear to be running Windows.";
        } else {
            ...
            if ($version_matches[1] != $phpv['major'] || $version_matches[2]  != $phpv['minor']) {
                $loader_php = $version_matches[1] . "." . $version_matches[2];
                $server_php =  $phpv['major'] . "." .  $phpv['minor'];
                $errors[ERROR_LOADER_WIN_PHP_MISMATCH] = "The installed loader is for PHP $loader_php but your Web Server is running PHP $server_php.";
            }
...
{
	// >>> WoodWing: never block during installation. The wwtest folder should be disabled after installation anyway.
	return false;
...
function default_page($loader_extension = LOADER_EXTENSION_NAME)
{
	...
	// WoodWing: Show intro for loader.
	echo '<p>Enterprise Server contains files that are protected by the '.
		'<a href="' . ENCODER_URL . '" target=encoder>ionCube PHP Encoder</a>. '.
    ...
    if (!is_supported_php_version()) {
        $msg = '<div class="alert">'.
            ...
            '<p>Please make sure that PHP 5.5 or 5.6 is installed.</p></div>';
        echo $msg;
	} else { // <<<
		...
		if( extension_loaded($loader_extension) ) { // BZ#31821 Removed check on Server Type (gives confusing error messages)
			$ionCubeLoaderOk = loader_already_installed($rtl);
		} else {
	...
	return $ionCubeLoaderOk; // WoodWing
}

function uninstall_wizard_instructions()
{
    // WoodWing: Removed misleading message; Not this script, but the testsuite.php script is shown in web browser.
...
{
	// >>> WoodWing: Enterprise Server 9.7+ requires ionCube Loader 5.0.14. Not older, not newer.
	$wrongVersionMsg = '';
	$lv = '';
	if( function_exists('ioncube_loader_iversion')) { // This function should be available with every loader
		$liv = ioncube_loader_iversion(); // version as integer
		$lv = sprintf("%d.%d.%d", $liv / 10000, ($liv / 100) % 100, $liv % 100);
		if ( version_compare( $lv, REQUIRED_LOADER_VERSION, '!=' ) ) { // 3 digit compare
			$wrongVersionMsg = 'ionCube Loader ' . $lv . '';
		}
	} else if( function_exists('ioncube_loader_version') ) { // Should never happen, but at least get version.
		$lv = ioncube_loader_version(); // version as string;
		$wrongVersionMsg = 'ionCube Loader ' . $lv . '';
	} else {
		// This should never happen, but when the loader doesn't support the ionCube loader API
		// give a proper error message. This could happen with very old loaders.
		$wrongVersionMsg = 'The system could not determine the ionCube Loader version.';
	}

	if( $wrongVersionMsg ) {
        echo '<div class="alert">'.
				'Please note that an <strong>incorrect</strong> version of ionCube Loader is currently installed:'.
				'<li>Installed version: '.$wrongVersionMsg.'</li>'.
				'<li>Required version: ionCube Loader '.REQUIRED_LOADER_VERSION.'</li>'.
				'<p>Install the required version by following the steps below.</p>'.
			'</div>';
		loader_not_installed();
		$ionCubeLoaderOk = false;
        ...
        echo '</div>';
        $ionCubeLoaderOk = true;
	}
	successful_install_end_instructions($rtl);
	return $ionCubeLoaderOk;
...
function loader_not_installed()
{
    ...
    if ($host_type != SERVER_UNKNOWN && is_array($loader) && !$sysinfo['DEBUG_BUILD']) {
        ...
        if (!empty($errors)) {
            ...
            // WoodWing: Added extra explaination.
            if (count($errors) > 1) {
	            echo '<p>Resolve these issues by installing ionCube Loader '.REQUIRED_LOADER_VERSION.' as explained below.</p>';
	        } else {
	            echo '<p>Resolve this issue by installing ionCube Loader '.REQUIRED_LOADER_VERSION.' as explained below.</p>';
	        }
	        // <<<
            echo '</div>';
        }
...
// >>> Woodwing: Already reported in loader_already_installed().
//     if (!isset($stype)) {
...
//     }
// <<<
// >>> WoodWing: Already handled elsewhere, see is_supported_php_version() call.
//     if (!is_supported_php_version()) {
...
// <<<
	if ($latest_supported_php_version = is_after_max_php_version_supported()) {
		echo '<strong>Your Web Server is running PHP version ' . PHP_VERSION . ' and is
         ...
         echo '<p>Your Web Server is currently running a debug build of PHP. The Loader cannot be installed with a debug build of PHP. Please ensure that PHP is reconfigured with debug disabled. Note that debug builds of PHP cannot help in debugging PHP scripts.</p>'; 
...
{
	// >>> WoodWing: redirect to the default page that handles all
	return default_page($ext_name);
//     heading();
// 
...
function heading()
{
	...
	-->
    <div id="main">
    <h2>ionCube Loader Wizard for Enterprise Server</h2> <!-- WoodWing -->
EOT;
}
...
EOT;
	...
	// >>> WoodWing: commented out, misleading
