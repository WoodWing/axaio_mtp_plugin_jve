<?php
/**
 * Base class for any 'class' that is drawable on html.
 *
...
abstract class HtmlBase
{
	...
	/**
	 * @var integer $BaseIdCounter Every instance that is created the static variable $BaseIdCounter is
	 * incremented so that every instance gets it's own $Id. This may be useful when streaming/referencing instances.
	 * Be careful to note that the baseidcounter does NOT allways represent the count of instances in memory, for
	 * example when an instance is deleted. It does represent the count of instances created.
	 */
	static protected $BaseIdCounter;

	/** @var string $Id  Every instance has its own unique id. */
	protected $Id;

	/**
	 * @var string $Owner representing the logical owner of the instance.  Can be null or else any instance derived
	 * from HtmlBase. Can be used to apply programming logic in an automated fashion, like  drawing instances only
	 * if and/or when the $Owner is drawn, etc.
	 */
	protected $Owner;

	/**
	 * @var string $Name Name that is used to identify the object in html. When a <input name=...>-field is created
	 * for example this is the name used. When the form is posted the same $Name is used (must be used) to retrieve
	 * the value.
	 */
	public $Name;

	/** @var string $ErrorString Holds an error message for display to the user. */
	public $ErrorString;
