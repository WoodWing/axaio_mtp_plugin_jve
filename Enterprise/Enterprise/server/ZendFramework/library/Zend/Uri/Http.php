...
class Zend_Uri_Http extends Zend_Uri
{
    ...
    public function validateQuery($query = null)
    {
        ...
        // Clean up the query for the things that are allowed. This is needed because otherwise
        // the preg_match will fail on MS Windows. The reason is that preg_match can lead
        // to an overflow of the stack size because of the nesting within the pattern.
        // See also: BZ#27134.
        // See also: https://bugs.php.net/bug.php?id=47689

        $pattern = '/%[[:xdigit:]]{2}/'; // %xx x = digit
        ...
        $pattern = '/(?:[' . self::CHAR_ALNUM . self::CHAR_MARK . self::CHAR_SEGMENT . '])/';
        // Letters and digits etc.. see __construct()
        $query = preg_replace( $pattern, '', $query, -1 );
        if ( is_null($query ) ) {
            require_once 'Zend/Uri/Exception.php';
            throw new Zend_Uri_Exception('Internal error: query validation failed');
        ...
        // Determine whether the remaining query is well-formed,
        // the $this->_regex['uric'] partly overlaps the previous two patterns.
