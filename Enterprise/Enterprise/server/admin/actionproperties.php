...
class ActionPropertiesAdminApp
{
	...
	{
		$rows = DBActionproperty::listActionpropertyGroups();
	...
	private function listNewAndCurrentActionProperties( $showMultiObj, $props, $rows, $detailTxt )
	{
		...
		if( $this->onlyQuery ) {
			$detailTxt .= '<tr><td>'.inputvar('order', '', 'small').'</td>';
	...
	{
		if( $this->onlyQuery ) {
			$txt = HtmlDocument::loadTemplate( 'actionpropertiesquery.htm' );
		} else {
			$txt = HtmlDocument::loadTemplate( 'actionproperties.htm' );
		}
	...
	public function buildCurrentActionProperties( $txt )
	{
		...
		$txt = str_replace("<!--KEY-->", "?publ=$this->publ&objtype=$this->objType", $txt);
