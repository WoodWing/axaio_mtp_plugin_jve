...
class PlnCreateLayoutsRequest
{
	...
	public function validate()
	{
		...
		$validator->enterPath( 'CreateLayoutsRequest' );
		...
		if( $validator->checkExist( $datObj, 'Layouts' ) ) {
			...
			if( !is_null( $datObj->Layouts ) ) {
				...
				if( !empty($datObj->Layouts) ) foreach( $datObj->Layouts as $listItem ) {
					$validator->enterPath( 'LayoutFromTemplate' );
					$validator->checkType( $listItem, 'PlnLayoutFromTemplate' );
					PlnLayoutFromTemplateValidator::validate( $validator, $listItem );
					$validator->leavePath();
				}
			}
			$validator->leavePath();
	...
	public function getASClassName() { return AS_CLASSNAME_PREFIX.'.pln.PlnCreateLayoutsRequest'; } // AMF object type mapping
