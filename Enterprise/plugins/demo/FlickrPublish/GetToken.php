...
if(!empty($_REQUEST['frob']) && !empty($_REQUEST['logon'])) {
	...
	if( defined('FLICKRPUBLISH_TOKEN') ){
		...
		echo "the FLICKRPUBLISH_TOKEN value in " . dirname(__FILE__) . "/config.php";
		echo "New Authorized Token: " . $token;
	}
	else {
		$define = "\ndefine ('FLICKRPUBLISH_TOKEN',		'" . $token . "' );";
...
elseif($_REQUEST['logon']) {
	$frob 	= $Flickr->getFrob();
	$url 	= $Flickr->getAuthUrl('delete', $frob);
?>
			...
			<form action="gettoken.php" method="post" name="frm" onSubmit="return true;">
