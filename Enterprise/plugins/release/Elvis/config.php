 ...
 * When the connection is lost, a message is shown to inform the user.
 *
 ...
 * Using a long time-out period may confuse Enterprise users because this could lead to a situation where the connection
 * is lost while the user is not yet informed about this. During this period, the user may retry or abort the action where
 * instead they should be patient and should be informed by the system.
 *
 * Using a short time-out period could lead to Elvis server connection errors for no valid reason (for example because an Elvis
 * server node is busy serving other user requests). Workflow operations that break halfway because of Elvis connection errors
 * is something that should be avoided because it may lead to users retrying the same operation over and over again, causing more
 * stress on the Elvis nodes (that seem to be busy already in this case).
 *
 * Note that this option works in conjunction with the ELVIS_CONNECTION_REATTEMPTS option. Both options together will
 * define the maximum time the user may have to wait before the Elvis connection error is thrown. For 5 reattempts and a
 * connection time-out of 3 seconds, the wait time could be 5x3=15 seconds.
 *
 * The Enterprise-Elvis communication is synchronous: the user waits for Enterprise and Enterprise waits for Elvis.
 * Any Load Balancer that is used (such as ELB on Amazon AWS) must support sticky sessions to stick a certain Enterprise session
 * to an Elvis session.
 * Basically, the whole route from the user via Enterprise to Elvis is sticky. But, when an Elvis node suddenly becomes unhealthy,
 * this route should change; Enterprise should pick a healthy Elvis node to continue with.
 * This should happen silently (without the user noticing) and is crucial for a stable integration. The next paragraph describes
 * how this works and what options are important to make this happen.
 *
 * When any kind of fatal communication error occurs (HTTP 5xx, time-out, and so on) Enterprise will interpret this as a connection
 * error. In case the error is returned immediately, it will wait for the configured time-out seconds. It will then try to
 * re-connect to Elvis for ELVIS_CONNECTION_REATTEMPTS times.
 * This is done by a re-login service call, which clears the Elvis session cookie that was set on the connection.
 * That releases the stickiness and gives the Load Balancer the opportunity to redirect the request to a different Elvis node than
 * the one used before, in the hope that node is healthy. In the meantime, the Load Balancer continuously polls all Elvis nodes regardless
 ...
 * The idea is that the Load Balancer should be able to find out that the Elvis node became unhealthy before the last re-logon attempt
 * took place. In that case the Elvis connection error should never occur, even when Enterprise was initially talking to an Elvis node
 ...
 * When the node hangs, there could be an additional 3-second time-out. So it will then find out in 10+3=13 seconds latest.
